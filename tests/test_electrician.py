"""Tests for the electrician simulator."""
...
def test_electrician_registered():
    sim = load_simulator("electrician")
    assert isinstance(sim, ElectricianSimulator)


def test_electrician_emits_expected_event_types():
    ...
    for expected in ElectricianSimulator().available_event_types():
        assert expected in emitted_types, f"missing {expected}"


def test_electrician_determinism():
    config = SimulatorConfig(
        business_type="electrician",
...
def test_electrician_state_snapshot():
    config = SimulatorConfig(
        business_type="electrician",
...
def test_electrician_runner_smoke():
    config = SimulatorConfig(
        business_type="electrician",
    ...
    count = runner.run()
    assert count >= 2
    assert all(e.business_type == "electrician" for e in adapter.events)
