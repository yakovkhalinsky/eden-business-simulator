...
- New events were gated by the existing `WeightedEventCatalog` patterns and added to each simulator's `available_event_types()` so that documentation tests and event coverage remain consistent.
