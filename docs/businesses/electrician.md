...
Slug: `electrician`  
Source module: [`src/eden_business_simulator/businesses/electrician.py`](../../src/eden_business_simulator/businesses/electrician.py)

## Domain overview

The electrician simulator models dispatched electricians who visit customer sites to install, repair, and inspect electrical systems. Tickets are created, electricians are assigned and dispatched, they arrive, diagnose issues, use parts, complete work, capture customer sign-off, generate invoices, receive payments, schedule follow-ups, and reorder truck stock.

## Systems represented

- Service ticket / work order creation
- Dispatch board and electrician mobile app
- GPS vehicle tracking
- Electrical parts inventory and reordering
...
1. Seed electricians, vehicles, customers, and an electrical parts catalog.
2. Create service tickets linked to customers and optional contracts.
3. Assign an electrician and vehicle to each ticket.
4. Dispatch and track the electrician to the site.
...
```bash
uv run eden-business-simulator run electrician --duration 300 --rate 2 --seed 42 --no-realtime

uv run eden-business-simulator daemon electrician \
  --stream-id electrician_seed42 \
  --storage sqlite --storage-uri electrician.db \
  --duration 600 --rate 2 --no-realtime

uv run eden-business-simulator replay electrician_seed42 \
  --storage sqlite --storage-uri electrician.db --speed 1.0
```
